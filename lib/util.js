...
function showDialog(name) {
    ...
    if (dialog.length === 1 )
    {
        ...
        $('#dialog_shield').show().add(name + ' .btn_dialog_close')
            .click(function(){ hideDialog(name); });
    } else {
...
function updateShareUrls(element, currentUrl) {
    ...
    element.find('.twitter')
      .attr('href', 'http://twitter.com/share?url=' + currentUrl);
    element.find('.facebook')
      .attr('href', 'http://www.facebook.com/sharer.php?u=' + currentUrl);
    element.find('.reddit')
      .attr('href', 'http://www.reddit.com/submit?url=' + currentUrl);
}
