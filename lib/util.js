...
function updateShareUrls(element, currentUrl) {
    ...
    $.each(['twitter', 'facebook', 'reddit', 'gplus', 'emails', 'views'], function(i, selector) {
        total += ( stats[selector] || 0 )
        element.find('.' + selector + '.like_button .count').html( stats[selector] || "&nbsp;");
    });
    // If stats are all zero, hide the area where the counters would be
    if (total === 0) { element.find('.like_button .count').hide(); }
...
//    clip.setText($('#expression_url').html());
//    clip.glue('btn_url_copy', 'btn_url_copy_wrapper');
}
