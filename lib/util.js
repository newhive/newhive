...
var append_color_picker = function(e, callback, init_color) {
    ...
    saturated_color = function(n, max) {
        if(!max) max = 1536;
        if(n < 0) n = 0;
        if(n > max) n = max;
    ...
    var make_picker = function(c) {
        ...
        d.css('background-color', c).attr('val', c).click(function() { callback(c) });
        return d.get(0);
    }
    ...
    var bar = $("<img style='width : 10px; height : 165px; position : absolute; top : 0px; left : 162px'>");
    bar.attr('src', '/lib/skin/1/saturated.png');
    var shades = $("<div style='width : 120px; height : 120px; position : absolute; top : 5px; left : 190px'><img src='/lib/skin/1/greys.png' style='width : 100%'></div>");
    ...
    var get_hue = function(e) {
        scolor = saturated_color(Math.floor(e.pageY - bar.offset().top), 164);
...
var background_pick = function() {
    ...
    append_color_picker($('#color_pick'), function(c) { $('#bg').css('background-color', c); }, Background.color);
    center(e);
}
