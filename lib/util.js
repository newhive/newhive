...
function hover_add(o) {
    ...
    else if($(o).css('background-image')) {
...
hover_menu = function(handle, drawer) {
    ...
    o.open = function() {
        ...
        if(!o.opened) {
            ...
            var maybe_top = hp.top + handle.outerHeight() + 5;
            var top = maybe_top + drawer.outerHeight() > $(window).height() ?
                hp.top - drawer.outerHeight() - 20 : maybe_top;
            drawer.css({ left : hp.left, top : top });
        }
    }
    
    handle.hover(o.open, o.delayed_close);
    drawer.hover(o.cancel_close, o.delayed_close);
    drawer.click(o.close);
    handle.click(function() { o.close() });

    return o;
}
click_menu = function(handle, drawer) {
    if(!handle.length) throw("no handle"); if(!drawer.length) throw("no drawer");
    var o = { handle : handle, drawer : drawer };
    handle.get(0).hover_menu = o;
...
tool_tip = function(tool, tip, above) {
    ...
    $(document.body).append(o.drawer);

    o.show = function() {
        var x = tool.offset().left + tool.width() / 2 - o.drawer.width() / 2;
        var y = o.tool.offset().top + o.tool.outerHeight() + 5; 
