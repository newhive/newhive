...
Hive.App.Html = function(common) {
    ...
    o.content = function() { return o.code.val() }
    ...
    o.toggle_render = function() { o.render(!o.rendered) }

    o.code = $("<textarea>").css({ width : '100%', height : '100%', border : 'none', 'z-index' : 2, padding : 0, margin : 0 });
    o.div.append(o.code);
    if(o.state.content) o.code.val(o.state.content);
    o.on_unfocus();
    o.setShield();

    setTimeout(function(){ common.load(); }, 100);
...
Hive.App.Image.Controls = function(common) {
    ...
    d.find('.resize, .rotate').drag('start', o.dragstart);
    d.find('.resize').drag(function(e, dd) {
        //cos(atan2(x, y) - atan2(w, h))
        o.app.resize([o.refDims[0] + dd.deltaX, o.refDims[1] + dd.deltaY]);
    });
...
var main = function() {
    ...
    $('#insert_text,#text_default').click(function(e) { return Hive.new_app({
        type : 'hive.text', content : ' ' }); });
    $('#text_header').click(function(e) {
        return Hive.new_app({ type : 'hive.text', content : ' ', scale : 2,
    ...
    $('#insert_html').click(function(e) { return Hive.new_app({type : 'hive.html'}) });
...
Hive.upload_start = function() {
    $('#loading').show();
}
Hive.upload_finish = function() {
    $('#loading').hide();
}
