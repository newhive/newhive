...
Hive.App.Controls = function(app) {
    ...
    o.append_link_picker = function(d) {
        ...
        var link = function() {
            // TODO: somehow guess if http:// should be prepended
            //var url = input.val();
            //if(
...
var main = function() {
    ...
    if(!/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent))
        alert("The New Hive expression editor is in beta, and right now it works best with Firefox. If you still want to try editing in your current browser, press OK.");
    ...
    $('#menu_save #title').change( function(){
        if ($('#url').val() === ""){
            $('#url').val(
...
Hive.save = function() {
    var on_response = function(ret) {
        ...
        if(ret.error) alert(ret.error);
            ...
            $('#dia_share_shield, #btn_dialog_close').unbind('click').click(hideDialogAndRedirect);
            $('#dia_share_shield').show().click(function(event){event.stopPropagation();});
            $('#expression_url').html(ret.location);
            $('#congrats_message').html('<h1><strong>Great work!</strong> Now you can share your expression anywhere.</h1>');
...
Hive.rte = function(options) {
    ...
    o.range_all = function() {
        var r = o.win.document.createRange();
        r.setStart(o.doc.body, 0);
        var last = o.doc.body;
