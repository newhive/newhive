...
Hive.App = function(initState) {
    ...
    o.getState = function() {
        ...
        if(o.angle) state.angle = o.angle();
    ...
    o.pos = function(pos) {
        ...
        else o.div.css({ 'left' : pos[0], 'top' : pos[1] });
    }
    ...
    o.pos(o.state.position);
    o.div.width(o.state.dimensions[0]).height(o.state.dimensions[1]);
...
Hive.App.Text = function(common) {
    ...
    var scale = o.state.scale ? o.state.scale : 1;
    o.scale = function(s) {
...
Hive.App.Controls = function(app) {
    ...
    o.layout = function() {
        ...
        o.div.width(d[0]);
        o.div.height(d[1]);
        o.select_box.width(d[0]);
        o.select_box.height(d[1]);
        o.c.buttons.css({ left  :  -5 - p, top : d[1] + p + 10 });
    }
...
Hive.App.Image = function(common) {
    ...
    var src = o.state.content;
    if(!src) src = '/lib/placeholder.png';
    ...
    o.imageResize = function(wh) { // wh :: [Integer, Integer]
        var newWidth = wh[1] * o.aspectRatio;
        var nwh = newWidth < wh[0] ? [newWidth, wh[1]] : [wh[0], wh[0] / o.aspectRatio];
        o.img.width(nwh[0]);
        o.img.height(nwh[1]);
        return nwh;
    }
    o.resize = function(dims1) {
        var dims2 = o.imageResize(dims1);
        common.resize(dims2);
        return dims2;
    }
...
var main = function() {
    ...
    $('#insert_text').click(function(e) {
        var app = Hive.App({ type : 'hive.text' });
        return false;
        //setTimeout(function() { app.focus() }, 200);
    });
    ...
    $('#insert_shape').click(function() {
        var app = Hive.App({ type : 'hive.shape.0', content : { type : 'rectangle' } });
        //setTimeout(function() { app.focus() }, 200);
    });
    
    //$('#btn_edit').click(function() { set_editing(true) });
    
    var up = new AjaxUpload('insert_image', {
         //action     : '/' + Hive.Exp.path
         name       : 'userfile'
        ,onComplete : Hive.imageUploaded
        });
    up.setData({ action : 'save_files' });
    
    load_state();
}
$(main);

function set_editing(editing) {
    // bad idea, should not be stored on server
    //Hive.Exp.editing = editing;
    //load_state();
}

function save() {
    on_response = function(url) {
        if(! url) alert("There was a problem saving your stuff :(.");
        window.location = url;
    }
    ExpressionUpload(get_state(), on_response);
    set_editing(false);
}
...
function ExpressionUpload(exp, on_response) {
    console.warn('saving');
    $.ajax( {
        type : "POST",
        //url : '/' + Hive.Exp.path,
        dataType : 'json',
        data : { action : 'save_exp', exp : JSON.stringify(exp), path : $('#url').val() },
        success : on_response
    } );
}

Hive.imageUploaded = function(f, response) {
  urls = JSON.parse(response);

  var img = $('<img>').attr('src', urls[0]);
  img.load(function() {
    img.remove();
    Hive.App({ type: 'hive.image', content: urls[0] });
  });
...
Hive.rte = function(options) {
    ...
    o.range_all = function(w) {
        ...
        r.setStart(w.document.body.firstChild, 0);
