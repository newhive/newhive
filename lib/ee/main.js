...
Hive.save = function() {
    var hide_dialog = function(){
        $('#btn_share').show();
        $('#dia_share').css('left', 'auto').animate({ 'top': 0, 'right': '10px'})
            .find('h1,h2,h3,div,img,a,pre,iframe').andSelf()
            //.animate({'width':0, 'height':0, 'font-size': 0, 'opacity':0});
            .animate({'width':0, 'height':0, 'font-size': 0, 'paddingLeft':0, 'paddingRight':0, 'paddingTop':0, 'paddingBottom':0});
    }
    var on_response = function(ret) {
        ...
        if(ret.error) alert(ret.error);
            ...
            center(
                $('#dia_share').find('pre').html(ret.location).end().show()
            );
            $('#btn_close_dialog').click(function(){hide_dialog('1000px', 0)});
