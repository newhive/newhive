...
Hive.App = function(initState) {
    ...
    o.getState = function() {
        var state = {
             type       :  o.type.tname
            ,content    :  o.content()
            ,position   :  o.pos_normalized
            ,dimensions :  o.dims_normalized
            ,z          :  o.layer()
        }
        if(o.scale_normalized) state.scale = o.scale_normalized;
    ...
    o.pos = function(pos) {
        if(typeof(pos) == 'undefined')
            ...
            o.pos_normalized = [pos[0] * o.sf(), pos[1] * o.sf()];
        }
    }
...
var main = function() {
    ...
    $(window).resize(function(e) {
        map(function(a) {
                a.pos_n(a.pos_normalized);
                a.dims_n(a.dims_normalized);
                a.scale_n(a.scale_normalized)
    ...
    $('#text_header').click(function(e) {
        var app = Hive.App({ type : 'hive.text', scale : 2,
            load : function() { app.rte.edit('bold') } });
        return false;
    });
    ...
    var up = new AjaxUpload('image_upload', {
         //action     : '/' + Hive.Exp.path
         name       : 'userfile'
        ,onComplete : Hive.imageUploaded
        });
    up.setData({ action : 'files_create', domain : $('#domain').val() });
...
function save() {
    ...
    $.ajax( {
        ...
        //url : '/' + Hive.Exp.path,
        dataType : 'json',
        data : { action : 'expr_save', exp : JSON.stringify(Hive.Exp) },
        success : on_response
    } );
}
