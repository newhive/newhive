...
Hive.save = function() {
    var hide_dialog = function(){
        ...
        var flashButton = function(){
            for (i=0; i<7; i++){
                btnShare.animate({'backgroundColor': "#f2f2f2"}, 150);
                btnShare.animate({'backgroundColor': "yellow"}, 150); 
            }
        }
        $('#dia_share').removeClass('border')
            .animate({'top':0, 'left':$(window).width() - 100}, {'duration': animationDuration, 'complete':flashButton})
            .find('h1,h2,h3,pre')
    ...
    var on_response = function(ret) {
        ...
        if(ret.error) alert(ret.error);
            ...
            $('#btn_close_dialog').click(function(){hide_dialog()});
